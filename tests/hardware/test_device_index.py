...
def test_returns_expected_properties(backend, device_id):
  device_index = xmipp4.hardware.DeviceIndex(backend, device_id)
  assert (
    (
...
def test_raises_type_error_with_invalid_arguments(backend, device_id):
  with pytest.raises(TypeError):
    xmipp4.hardware.DeviceIndex(backend, device_id)
...
def test_returns_expected_properties_from_string(backend, device_id):
  device_index = xmipp4.hardware.DeviceIndex(f"{backend}:{device_id}")
  assert (
    (
      device_index.backend, device_index.id
    ) == (backend, device_id)
...
def test_raises_value_error_with_invalid_string(name):
  with pytest.raises(ValueError):
    xmipp4.hardware.DeviceIndex(name)
...
def test_are_equals(backend, device_id, are_equals):
  d1 = xmipp4.hardware.DeviceIndex(backend, device_id)
  d2 = xmipp4.hardware.DeviceIndex('cuda', 1)
...
def test_are_not_equals(backend, device_id, are_not_equals):
  d1 = xmipp4.hardware.DeviceIndex(backend, device_id)
  d2 = xmipp4.hardware.DeviceIndex('host', 1)
...
def test_pickle():
  di = xmipp4.hardware.DeviceIndex("host", 0)
